...
00_init_app.py              --项目初始化（检测版本、库安装，数据库初始化等）
01_image_processor.py       --图片预处理（批量重命名、写入数据库等操作）
02_exif_extractor.py        --提取图片EXIF信息
03-b_yolo-seg-mask.py       --基于yolo提取人物分割图片（用于模型训练）
03-z_yolo-pose_previews.py  --基于yolo提取人物姿态信息并可视化（展示用）
03-z_yolo-seg_previews.py   --基于yolo提取人物分割图片并可视化（展示用）
后续（待开发）

z00_clean_project.py        --项目格式化（删除数据库、日志等创建的文件）

exif_data/                  --存放提取出的图片EXIF数据的目录（如用户选择保存CSV文件）
exif_utils/                 --包含EXIF信息提取和处理逻辑的工具模块
config.json                 --项目的全局配置文件（数据库路径、GPU设置、线程数等）
...
*· 其他信息：图片规格 拍摄设备  镜头信息 快门速度 光圈值 感光度  EV值 白平衡 焦距*
